...
export const isCluster = (item: ResultItem): item is ResultItem<[SpinqueResultObject]> => {
  if (!item.tuple || item.tuple.length !== 1) {
    return false;
  }
  const tuple = item.tuple[0];

  if (typeof tuple === 'string' || typeof tuple === 'number' || !('class' in tuple) || !('id' in tuple)) {
    return false;
  }

  return (tuple as SpinqueResultObject).class.includes(RDFS_CLASS);
};
